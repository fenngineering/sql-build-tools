...
Import-LocalizedData -FileName "env.config.psd1" -BindingVariable "DBConfig" -UICulture "en-GB"

$template = "C:\dbgit\db-publish\ps-templates\SsisProjectParams\Currency.eps"                   

If (-Not (Test-path $template)) {
    Write-Output "$template does not exist."
    Continue
}

$environment = if($Env:environment -eq $null) {"dev"} else {$Env:environment}

Expand-Template -file $template